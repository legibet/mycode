...
`assistant.meta.stop_reason` uses the canonical values `stop`, `tool_use`, `length`, `error`, `cancelled`, and `unknown`. Provider adapters convert their native finish reasons before persistence. A record with `stop_reason="error"` also stores the `error` event's message in `assistant.meta.error`. `assistant.meta.model` records the selected request model, not a provider-returned alias or routed model name. A streamed tool block with unparseable JSON carries `meta.invalid_input=true` plus the original text in `meta.raw_arguments` and the parser message in `meta.parse_error`.
